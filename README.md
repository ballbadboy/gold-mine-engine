...
**Marketing Workspace + AI Admin — ระบบนำร่องสำหรับจัดการการตลาดและดูแลผู้เล่นเว็บเกม**
...
เพิ่ม **AI Admin Pilot** สำหรับบริการผู้เล่นเกมบันเทิง: หน้าแชต `/help` และพื้นที่ผู้ดูแล `/admin-ai` มีคลังคำตอบ กล่องรับเรื่อง และผู้ช่วยถามสรุปงาน เชื่อมกับพื้นที่ข้อมูลและบัญชีผู้ดูแลเดิม ทดลองแบบไม่เรียก AI ภายนอกได้
...
สารบัญ: [AI แอดมิน](#ai-admin) · [การทำงาน](#workflow) · [วิธีทดลอง](#demo) · [วิธีใช้แต่ละส่วน](#features) · [การตั้งค่า](#setup) · [เชื่อมระบบเกม](#webhook) · [สูตรวัดผล](#metrics) · [ข้อจำกัด](#limits) · [การทดสอบ](#validation) · [แผนต่อยอด](#roadmap)

<a id="ai-admin"></a>

## AI แอดมิน: ตอบผู้เล่นและช่วยงานหลังบ้าน

| ส่วนงาน                    | ทำได้แล้ว                                                                                                     | ยังต้องต่อเพิ่ม                                          |
| -------------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| แชตผู้เล่น `/help`         | ตอบจากคำตอบที่เผยแพร่ อ้างชื่อและรุ่นของคำตอบ ขอผู้ดูแล รับเรื่องเมื่อไม่รู้ และอ่านคำตอบต่อในเบราว์เซอร์เดิม | บัญชีผู้เล่นจริง ประวัติข้ามอุปกรณ์ ระบบแจ้งเตือนนอกเว็บ |
| กล่องรับเรื่อง `/admin-ai` | กรองสถานะ อ่านประวัติ รับช่วงตอบ ปิดเรื่อง และตอบใหม่ในเรื่องเดิม                                             | ทีมหลายคน การมอบหมายงาน SLA และการแจ้งเตือนผู้ดูแลภายนอก |
| คลังคำตอบ                  | เพิ่ม/แก้ฉบับร่าง เผยแพร่/พักคำตอบ วลีคำค้น และตรวจรุ่นก่อนเขียนทับ                                           | นำเข้าเอกสาร การอนุมัติหลายคน และการค้นจำนวนมาก          |
| ผู้ช่วยหลังบ้าน            | ถามจำนวนเรื่องค้าง ความพร้อมของคลังคำตอบ การใช้ AI และสรุปงานที่ควรติดตาม                                     | การเชื่อมข้อมูลหรือทำงานในหลังบ้านเกมจริง                |

**ขอบเขต AI ในรุ่นนี้:** โมเดลช่วยเลือกหัวข้อคำตอบและแยกประเภทคำถามของผู้ดูแล จากนั้นระบบส่งคำตอบที่เผยแพร่ไว้หรือคำนวณตัวเลขจากข้อมูลจริง ไม่มีเครื่องมือให้โมเดลเข้าถึงบัญชีผู้เล่น เปลี่ยนยอดเงิน แบนบัญชี ส่งโฆษณา หรือสั่งงานตามข้อความในแชต การตอบด้วยข้อความที่คนตรวจแล้วช่วยตรวจย้อนกลับได้ แต่การเลือกหัวข้อของ AI ยังอาจผิดเรื่อง ต้องทดสอบกับคำถามจริงก่อนเปิดใช้งาน

### ทดลองทั้งสองฝั่ง

1. เริ่มด้วย `pnpm demo` ตามวิธีติดตั้งด้านล่าง
2. เปิด [พื้นที่ AI แอดมินในเครื่อง](http://127.0.0.1:3100/admin-ai) และกด **โหลดข้อมูลแอดมินตัวอย่าง** เมื่อพื้นที่แอดมินยังว่าง ได้คำตอบตัวอย่าง 3 เรื่องและงานค้าง 1 เรื่อง
3. เปิด [หน้าแชตผู้เล่น](http://127.0.0.1:3100/help) พิมพ์ `เกมค้างครับ` ระบบจะตอบจากคลัง พร้อมระบุที่มา
4. ลองคำถามที่ไม่มีในคลังหรือกด **ขอผู้ดูแล** เรื่องจะเข้ากล่องผู้ดูแล ระบบไม่ได้ส่งแจ้งเตือน LINE/อีเมลให้เจ้าหน้าที่
5. กลับหน้าแอดมิน กด **รีเฟรช** เลือกเรื่อง แล้ว **รับช่วงดูแล → ส่งคำตอบ → ปิดเรื่อง** คำตอบจะปรากฏในหน้าแชตผู้เล่นเมื่อรีเฟรชหรือรอบตรวจข้อความทุก 10 วินาทีขณะเปิดหน้านั้นอยู่
6. เปิดแท็บ **ผู้ช่วยหลังบ้าน** ถาม `วันนี้มีอะไรต้องทำ` หรือ `คลังคำตอบพร้อมไหม` เพื่ออ่านสรุปจากข้อมูลล่าสุด

ข้อมูลตัวอย่างระบุว่าเป็นตัวอย่างและต้องปรับเป็นข้อมูลจริงของเกมก่อนใช้งาน รุ่นสาธิตปิดการเรียก AI ภายนอกเสมอ แม้เครื่องมี API key

### เตรียมคลังคำตอบ

- ใส่หัวข้อ คำตอบจริง และวลีเฉพาะ เช่น `เกมค้าง, โหลดไม่ขึ้น` จากนั้นบันทึกฉบับร่างและกด **เผยแพร่คำตอบ** เมื่อทบทวนแล้ว
- การแก้เนื้อหาจะกลับเป็นฉบับร่างทันที และต้องเผยแพร่ใหม่ ผู้เล่นจะไม่ได้รับฉบับร่าง
- หากพบวลีตรงกับคำตอบที่เผยแพร่เพียงเรื่องเดียว ระบบใช้คำตอบนั้นโดยไม่เรียกโมเดล วลีสำหรับเส้นทางนี้ต้องยาวอย่างน้อย 4 ตัวอักษร เลี่ยงคำกว้าง เช่น “เกม”
- หากไม่พบหรือพบหลายเรื่อง เมื่อเปิด AI แล้ว โมเดลจะเลือกได้เฉพาะ ID ของหัวข้อที่เผยแพร่ และระบบตรวจรุ่นของคำตอบซ้ำก่อนตอบ หากไม่แน่ใจ ไม่พบ ID ที่อนุญาต ข้อมูลเปลี่ยน หรือบริการขัดข้อง จะรับเรื่องให้คน
- แนะนำเริ่มจากวิธีใช้งาน อุปกรณ์ที่รองรับ ปัญหาการโหลด และวิธีแจ้งข้อผิดพลาด ไม่ใส่รหัสผ่าน คีย์ ข้อมูลบัญชี หรือข้อมูลส่วนตัวในคลัง

### เปิดใช้งานในเว็บและเชื่อม AI

ใช้ owner login และ Supabase ตามวิธีตั้งค่าของ workspace เดิม ข้อมูลแอดมินอยู่ใน `state.support` ของ `marketing_workspaces` จึงไม่ต้องเพิ่ม migration สำหรับส่วนนี้ พื้นที่เดิมที่ยังไม่มี `support` จะได้ค่าเริ่มต้นว่างโดยรักษาข้อมูลการตลาดไว้

เพิ่มใน `.env.local` หรือ environment ของ deployment:

```dotenv
# เปิดหน้าแชตเว็บเมื่อคลังคำตอบและผู้ดูแลพร้อมแล้ว
SUPPORT_PUBLIC_ENABLED=true
# ต้องเปิดเองจึงจะส่งคำถามออกไปยังผู้ให้บริการ AI
SUPPORT_AI_ENABLED=true
SUPPORT_AI_PROVIDER=gemini
GEMINI_API_KEY=replace_with_your_private_key
# เว้นว่างเพื่อใช้ default ของ provider; ตรวจชื่อโมเดลที่บัญชีใช้งานได้
SUPPORT_AI_MODEL=
# จำกัดจำนวนการเรียก ไม่ใช่วงเงินค่าใช้จ่าย
SUPPORT_AI_DAILY_LIMIT=50
```

รองรับตัวเชื่อมที่มีใน repo: `claude`/`ANTHROPIC_API_KEY`, `gemini`/`GEMINI_API_KEY`, `openrouter`/`OPENROUTER_API_KEY` เลือกหนึ่งตัวด้วย `SUPPORT_AI_PROVIDER` หรือเว้นว่างเพื่อใช้ตัวแรกที่ตั้งค่าไว้ เก็บคีย์นอก Git และบน server เท่านั้น คำสั่งเริ่มแอปของ repo ล้างคีย์ที่สืบทอดจาก shell; สำหรับเครื่องพัฒนาให้ใส่ใน `.env.local` ตามรูปแบบเดิม

ค่าเริ่มต้น `SUPPORT_PUBLIC_ENABLED=false` และ `SUPPORT_AI_ENABLED=false` หน้าแอดมินใช้ได้หลังเข้าสู่ระบบ แต่ API แชตผู้เล่นตอบ 404 จนกว่าจะเปิดใช้งาน หากไม่เปิด AI หรือไม่มี provider ที่เลือกไว้ ระบบยังจับคู่คำตอบและรับเรื่องให้คนได้ **การตั้ง API key ไม่ได้เชื่อม LINE, Facebook Messenger หรือระบบเกมโดยอัตโนมัติ**

### ข้อมูลที่ใช้ ขีดจำกัด และการส่งต่อ

- ผู้เล่นมีแชตนิรนามหนึ่งเรื่องต่อคุกกี้เบราว์เซอร์ คุกกี้เป็น HttpOnly/SameSite และเก็บเฉพาะ hash ของ secret ในฐานข้อมูล ไม่ถือเป็นการยืนยันตัวตนเจ้าของบัญชีเกม แอดมินห้ามใช้แชตนี้เป็นหลักฐานอนุมัติเรื่องบัญชี
- ระบบปกปิดอีเมล หมายเลขยาว และค่าที่ตามหลังรูปแบบ password/OTP บางรูปแบบก่อนบันทึกและส่งให้โมเดล การปกปิดนี้ไม่ครอบคลุมข้อมูลส่วนบุคคลทุกรูปแบบ เช่น ชื่อหรือที่อยู่ จึงต้องกำหนดนโยบายข้อมูลและข้อความแจ้งผู้ใช้ของเว็บไซต์จริง
- โมเดลได้รับเฉพาะคำถามที่ปกปิดแล้วและชื่อ/คำค้น/ID ของคำตอบที่เผยแพร่ สำหรับผู้ช่วยหลังบ้านส่งเฉพาะคำถามของผู้ดูแล ไม่ส่งประวัติแชตทั้งชุด ข้อมูลการตลาด หรือข้อมูลธุรกรรม
- ผู้เล่นเห็นเฉพาะแชตของตนผ่าน session ส่วนแอดมินเห็นกล่องรับเรื่องทั้งหมดหลังเข้าสู่ระบบ ทุกการเขียนต้องมาจาก Origin ของแอป ไม่เปิด cross-origin widget ในรุ่นนี้
- เรื่องบัญชี รหัสผ่าน การเงิน ความปลอดภัย และการขอคนจะส่งต่อผู้ดูแล ไม่มีการทำธุรกรรม หลังคนรับช่วงหรือส่งคำตอบแล้ว บอตจะไม่กลับมาแทรกตอบในเรื่องนั้น
- เก็บข้อความบนระบบศูนย์ช่วยเหลือ 30 วันนับจากเปิดเรื่อง และลบจาก active snapshot เมื่อมีการอ่าน/เขียนส่วนแอดมินครั้งถัดไป ไม่มี scheduled deletion หรือการลบสำเนาสำรองอัตโนมัติในรุ่นนี้ ประวัติคำสั่งผู้ดูแลเก็บล่าสุด 200 รายการโดยไม่บันทึกเนื้อหาข้อความ
- คลังสูงสุด 50 คำตอบ, บทสนทนาสูงสุด 500 เรื่อง, เปิดใหม่สูงสุด 100 เรื่อง/วัน UTC, รับข้อความผู้เล่นรวม 1,000 ข้อความ/วัน และ 30 ข้อความต่อเรื่อง เว้นระยะส่ง 2 วินาที ประวัติรวมสูงสุด 100 ข้อความต่อเรื่อง ขีดจำกัดเป็นของ workspace ร่วมกัน จึงยังต้องเพิ่มการป้องกันบอตและ rate limit ตามผู้ใช้/IP ก่อนรับ traffic สูง
- ค่าเรียก AI เริ่มต้นจำกัด 50 ครั้ง/วัน UTC ตั้งได้ 0–1,000 ครั้ง การผิดพลาด/timeout นับด้วย จองโควตาแบบ atomic ก่อนเรียกและไม่มีการลอง provider อื่นอัตโนมัติ กำหนด timeout 10 วินาที; หาก process สะดุด เรื่องที่ค้างเกิน 20 วินาทีจะส่งต่อคนในการตรวจครั้งถัดไป การยกเลิก request ไม่รับประกันว่าผู้ให้บริการจะไม่คิดค่าบริการ
- คำตอบจากโมเดลเป็น JSON เลือก ID/ประเภทงานเท่านั้น ไม่แสดงข้อความอิสระจากโมเดลต่อผู้เล่น และไม่มี tool execution คำขอที่ไม่รองรับจะเสนอหัวข้อที่ถามได้
- ข้อความซ้ำที่มี request ID เดิมไม่บันทึกซ้ำ การส่งคำตอบของคนใช้ request ID เช่นกัน คนรับช่วงหรือพักคำตอบขณะ AI กำลังทำงานมีสิทธิ์เหนือผล AI ที่กลับมาภายหลัง
- ข้อความแชตและ activity นับรวมกับขีดจำกัด 20,000 records / 8 MB ของ workspace เดิม ต้องแยกตารางเมื่อขยายระบบ

### API ของแอดมิน

ผลตรวจส่วนแอดมิน: 46 unit/database tests และ 19 HTTP checks รวม flow เดิม ผ่าน lint/TypeScript/build และตรวจหน้าจอแชต/ผู้ดูแลบน desktop กับ mobile ดูขอบเขตการทดสอบใน [Validation](docs/VALIDATION.md) การทดสอบโมเดลใช้ stub; ยังต้องประเมิน LLM จริงกับข้อมูลเกมก่อนใช้งาน

| Endpoint             | งาน                                                                                 | สิทธิ์                                                 |
| -------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------ |
| `GET /api/support`   | สถานะ คำถามที่เผยแพร่ และแชตของ session ปัจจุบัน                                    | ต้องเปิด public support; แชตต้องมี support cookie      |
| `POST /api/support`  | `start`, `message` (UUID `requestId`, `text`), `handoff`                            | Same-origin; `message`/`handoff` ต้องมี support cookie |
| `GET /api/admin-ai`  | คลังคำตอบ กล่องรับเรื่อง และสรุป                                                    | Owner หรือ local demo                                  |
| `POST /api/admin-ai` | `article.save`, `article.publish`, `ticket.reply`, `ticket.status`, `assistant.ask` | Owner + same-origin; `demo.seed` เฉพาะ local demo      |

**งานต่อยอด:** เชื่อมช่องทางแชตทีละช่องทาง, ผูกบัญชีผู้เล่นหลังยืนยันตัวตน, เพิ่มทีม/สิทธิ์, ระบบแจ้งเตือน, นำเข้าเอกสาร, การค้นหลายภาษา และประเมินคำถามจริงกับโมเดลที่เลือก รุ่นนี้ยังไม่ได้เชื่อม provider จริงหรือทดสอบคุณภาพคำตอบจากโมเดลจริง ผลทดสอบอัตโนมัติใช้ model stub และข้อมูลสังเคราะห์
